...
The context file is the entry point for every tidylake project. Whether you are using the [CLI](cli.md) or the [SDK](sdk.md), this file is the first thing parsed during a command or library import.

It defines how the package behaves and can be extended with custom functionality through then [plugin system](plugins/overview.md).
...
When this file is loaded, tidylake scans for the corresponding [manifest files](manifests.md). The only requirement is that these files exist and follow the standard YAML schema for manifests.

### Building the Dependency DAG {#dag}

The order in which you list products in the context file **does not matter**. Instead, the execution order is determined by the internal dependencies defined within your [script files](./scripts.md).
