...
As established in our [getting started](index.md) guide, the primary objective of tidylake is to solve the **Data Platform Balancing Act**.
...
## Encapsulating the data product lifecycle {#data-product}
...
2. **Metadata:** That explains what the data means and how it should be used.
3. **Contracts:** Explicit definitions that combine strict typing, semantics, and content restrictions.

By capturing this information in two compact files (the [manifest](manifests.md) and the [script](scripts.md)), the framework can agnostically automate almost every step of the lifecycle:
...
### The Notebook vs. Script Dilemma: Balancing Engineering and Analysis {#batch-vs-interactive}
...
Check out how [script files](scripts.md) can be configured to see how these principles work in practice:
...
We use a central [manifest](manifests.md) as the single source of truth. This single file serves as a __contract__ to automate several critical processes:

!!! info "Schema Enforcement & Quality" 
...
!!! info "Automated Catalog Maintenance" 
    Using a [compute engine plugin](plugins/compute-engine.md#schema-automation), the CLI can automate table creation and schema evolution in your metastore. This separates your data lifecycle from your catalog maintenance—essential for modern formats like Delta Lake or Apache Iceberg.

!!! info "Testing with Synthetic Data" 
...
!!! info "Orchestration Agnosticism" 
